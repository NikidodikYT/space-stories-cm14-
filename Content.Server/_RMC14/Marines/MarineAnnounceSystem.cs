...
public sealed class MarineAnnounceSystem : SharedMarineAnnounceSystem
{
        ...
        Filter? filter)
    {
        ...
        var uiFilter = filter == null
            ? BuildMarineAnnouncementFilter()
            : Filter.Empty().AddPlayers(filter.Recipients);

        // Mirror the radio recipients: rescued survivors do not receive command announcements.
    ...
    private Filter BuildMarineAnnouncementFilter()
    {
        return Filter.Empty().AddWhereAttachedEntity(e =>
            HasComp<MarineComponent>(e) || HasComp<GhostComponent>(e));
    }
